...
import numpy as np
class GPT2LM:
    def __init__(self, use_tf=False, device=None, little=False, cache_dir=None):
        """
        ...
        import transformers
        self.use_tf = use_tf
        # self.tokenizer = transformers.GPT2TokenizerFast.from_pretrained("gpt2-large",cache_dir=cache_dir)
        self.tokenizer = transformers.GPT2TokenizerFast.from_pretrained("gpt2-large")

        if use_tf:
            # self.lm = transformers.TFGPT2LMHeadModel.from_pretrained("gpt2",cache_dir=cache_dir)
            self.lm = transformers.TFGPT2LMHeadModel.from_pretrained("gpt2")

        else:
            # self.lm = transformers.GPT2LMHeadModel.from_pretrained("gpt2-large", cache_dir=cache_dir, from_tf=False)
    ...
    def __call__(self, sent):
        ...
        if self.use_tf:
            ...
            ipt = self.tokenizer(sent, return_tensors="tf", verbose=False,max_length=1024)
            ret = self.lm(ipt)[0]
            loss = 0
            ...
            ipt = self.tokenizer(sent, return_tensors="pt", verbose=False,max_length=1024)
            # print(ipt)
            # print(ipt.input_ids)
            try:
